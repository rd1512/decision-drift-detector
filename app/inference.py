...
def predict(features: list[float]) -> tuple[str, float, dict]:
    ...
    input_summary = {
        "mean": float(input_array.mean()),
        "std": float(input_array.std())
    }
    ...
    return decision, round(float(probability), 4), input_summary
